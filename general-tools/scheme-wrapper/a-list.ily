...
% a little helper to warn with or without location
#(define (warn location fmt . vals)
   (if (ly:input-location? location)
       (apply ly:input-warning location fmt vals)
       (apply ly:warning fmt vals)
))
...
#(define-public (get-a-tree parser location name path)
   ...
   (let ((opts (ly:parser-lookup parser name)))
     ...
     (if (list? opts)
         (getval opts path)
         (begin
          (warn location "~A is not list (~A)" name opts)
          #f)
         )))
      ...
      (let ((sym (car op))
        ...
        (if (> (length op) 1)
            ...
            (let ((ov (assoc-get sym ol #f)))
              (if ov (warn location "deleting '~A'" ov))
